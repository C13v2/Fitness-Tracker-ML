...
def count_reps(dataset, cutoff=0.4,order=10,column="acc_r"):
    data = LowPass.low_pass_filter(
        squat_set, col=column, sampling_frequency=fs, cutoff_frequency=cutoff, order=order
    )

    indexes = argrelextrema(data[column + "_lowpass"].value, np.greater)
    ...
    esercise = dataset["label"].iloc[0].title()
...
for s in df["set"].unique():
    ...
    rep_df.iloc[rep_df["set"] == s, "reps_pred"] = reps
